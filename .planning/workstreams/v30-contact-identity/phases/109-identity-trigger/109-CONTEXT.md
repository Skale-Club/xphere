...
- **D-01:** Three triggers, three SQL functions:
  1. `enforce_contact_identity_at_commit` — `CREATE CONSTRAINT TRIGGER ... AFTER INSERT OR UPDATE ON contacts DEFERRABLE INITIALLY DEFERRED FOR EACH ROW`. Function checks `phone_e164 IS NULL AND email_normalized IS NULL AND identity_status <> 'archived_duplicate' AND identity_status <> 'channel_only' AND NOT EXISTS (SELECT 1 FROM contact_channel_identities WHERE contact_id = NEW.id)` → RAISE EXCEPTION. **Status-based skip per RESEARCH.md Option A:** Phase 108 webhooks use 2 separate transactions (Supabase JS doesn't expose BEGIN/COMMIT), so DEFERRABLE alone wouldn't help. The `channel_only` status acts as the "I owe you an identity" promise — rows with that status bypass the immediate check, the channel identity then arrives in the next webhook step. DEFERRABLE is retained for future RPC-based callers that can wrap both inserts in one transaction.
