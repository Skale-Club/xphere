...
autonomous: true
requirements: [REENG-01, REENG-03, REENG-08, REENG-18]
requirements_addressed: [REENG-01, REENG-03, REENG-08, REENG-18]
must_haves:
  truths:
  ...
  artifacts:
    ...
    - path: "tests/ghl-reengagement-route.test.ts"
      provides: "REENG-05, REENG-06, REENG-07, REENG-15, REENG-16, REENG-18 route-handler scaffold (incl. DB schedule check + ?force=1 bypass + reschedule UPDATE)"
...
```
  ...
  <acceptance_criteria>
    ...
    - File contains `describe('POST /api/automations/ghl-reengagement/run (REENG-05, REENG-06, REENG-07, REENG-15, REENG-16, REENG-18)'
    - File contains the literal string `automation_schedules` (REENG-18 schedule check stubs)
    - File contains the literal string `'not_due_yet'` (REENG-18 schedule skip)
    - File contains the literal string `'inactive'` (REENG-18 is_active=false)
    - File contains the literal string `?force=1` (D-32-09 force bypass)
    - File contains the literal string `last_run_status` (REENG-18 reschedule UPDATE)
    - File contains at least 6 additional `it(...` blocks under the `REENG-18` section comment`
