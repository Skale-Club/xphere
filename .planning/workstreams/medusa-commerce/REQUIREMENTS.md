...
- [ ] **CHT-01**: `rate-limit.ts` gains `failMode: 'open'|'memory'|'closed'` (memory = per-instance token-bucket fallback; closed = deny when Redis down); existing call sites unchanged
