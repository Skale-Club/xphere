...
import type { PlatformSettingEntry } from '@/lib/platform-keys'
...
type EmailSettingsSummary = Omit<PlatformEmailSettingsRow, 'api_key_encrypted'> & { key_hint?: string }
...
interface Props {
  stats: PlatformStats
  globalSettings: PlatformSettingEntry[]
...
export function PlatformSettingsView({ stats, globalSettings, emailSettings }: Props) {
  ...
  const aiSettings = globalSettings.filter((s) => s.tab === 'AI provider')
    ...
    <div className="p-4 sm:p-6">
      ...
      <Tabs defaultValue="overview">
        ...
        <TabsContent value="email" className="space-y-4">
          <PlatformEmailForm initial={emailSettings} />
        </TabsContent>
      </Tabs>
    </div>
  )
