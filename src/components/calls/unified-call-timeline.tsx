      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <PillGroup
            ...
            current={currentType}
            onSelect={(v) => setParam('type', v === 'all' ? null : v)}
          ...
          <PillGroup
            ...
            onSelect={(v) => setParam('direction', v === 'all' ? null : v)}
          ...
          {groups.map((group) => (
            <div key={group.key} className="space-y-2">
              ...
              <div className="flex flex-col gap-1.5">
                {group.rows.map((row) => (
                  <UnifiedCallRow key={`${row.call_type}-${row.id}`} row={row} />
                ))}
              </div>
            </div>
          ))}
...
function PillGroup<T extends string>({
  ...
  return (
    <div className="inline-flex rounded-[10px] border border-border bg-bg-secondary p-1" role="group" aria-label={ariaLabel}>
      {filters.map((f) => (
        <button
          key={f.id}
          type="button"
...
function UnifiedCallRow({ row }: { row: UnifiedCallWithContact }) {
  ...
  const displayName = row.contact?.name
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-0.5 flex items-center gap-2 text-[11.5px] text-text-tertiary">
          ...
          {row.call_type === 'ai' && row.cost != null && Number(row.cost) > 0 && (
            <>
              <span>·</span>
