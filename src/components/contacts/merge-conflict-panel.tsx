...
import { mergeContactAction, getPendingMergeConflict, type MergeConflictPair } from '@/app/(dashboard)/contacts/actions'
  ...
  return (
    ...
    >
      <div className="mb-2 flex items-center justify-between">
        ...
        {isWinner && (
          <Badge variant="secondary" className="text-[10px] px-1.5 py-0.5 text-accent bg-accent-muted">Keep</Badge>
        )}
      </div>
...
/**
 * Banner + sheet for contacts with identity_status = 'merge_conflict'.
 *
  ...
  return (
    <>
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent side="right" className="w-full sm:max-w-[520px] flex flex-col overflow-hidden">
          <SheetHeader className="shrink-0">
            <SheetTitle>Resolve duplicate contact</SheetTitle>
            <SheetDescription>
              Choose which contact to keep. The other will be archived and its
              conversations, notes, and tags moved to the survivor.
            </SheetDescription>
          </SheetHeader>
            ...
            {loadingPair ? (
              ...
              <div className="space-y-4">
                ...
                <div className="rounded-md border border-border bg-bg-secondary px-3 py-2 text-[12px] text-text-secondary">
                  ...
                  <span className="font-medium text-text-primary">status = archived_duplicate</span>.
                  All linked data (conversations, notes, tags, opportunities) will be moved
                  to the survivor.
                </div>
              </div>
            )}
          ...
          <div className="shrink-0 border-t border-border-subtle pt-4 flex items-center justify-end gap-2">
            ...
            <Button
              onClick={handleMerge}
              disabled={!pair || !winnerId || merging}
            >
              {merging ? (
                <>
