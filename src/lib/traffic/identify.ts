...
//
// Links an anonymous traffic visitor to a known contact so the CAPI sender can
// resolve the visitor's click signals (fbc/fbp/ip/ua) when a Lead/Purchase
// fires. Sets traffic_visitors.contact_id and back-fills recent un-linked
// traffic_events for that visitor. Service-role only (no RLS context here).
 ...
 * (stored in traffic_events.metadata.event_id on form_submit). Used to dedup
