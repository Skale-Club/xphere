...
async function queryTrafficSummary(input: Record<string, unknown>, ctx: ToolContext): Promise<ToolResult> {
...
async function queryTrafficSessions(input: Record<string, unknown>, ctx: ToolContext): Promise<ToolResult> {
...
async function queryTrafficCampaigns(input: Record<string, unknown>, ctx: ToolContext): Promise<ToolResult> {
  const days = Math.min(Number(input.days ?? 30), 90)
  const from = new Date(Date.now() - days * 864e5).toISOString()
...
export const trafficTools: CopilotToolRegistry = {
  query_traffic_summary: {
    mode: 'read',
    definition: {
      name: 'query_traffic_summary',
      description: 'Get a summary of website traffic: visitors, sessions, page views, conversions, top sources and campaigns. Default period is 30 days.',
      input_schema: {
        type: 'object',
        properties: {
    ...
    handler: queryTrafficSummary,
  },
  query_traffic_sessions: {
    mode: 'read',
    definition: {
      name: 'query_traffic_sessions',
    ...
    handler: queryTrafficSessions,
  },
  query_traffic_campaigns: {
    mode: 'read',
    definition: {
      name: 'query_traffic_campaigns',
