...
const CHANNEL_TABS: { value: string; label: string; icon: React.ComponentType<{ className?: string }> }[] = [
...
async function getProviderAvailability() {
  ...
  return {
    ...
    hasResend: false, // resend is not yet in the integrations provider enum; gated
  }
}
            ...
            {campaigns.map((c) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Actions */}
                  <CampaignActions campaignId={c.id} campaignStatus={c.status} campaignChannel={c.channel as CampaignChannel} />
                </div>
              )
            })}
          </div>
