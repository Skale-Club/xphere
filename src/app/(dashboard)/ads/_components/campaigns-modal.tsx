...
export function CampaignsModal({
  ...
  // Reset to the top level whenever the modal is (re)opened.
  useEffect(() => {
  ...
  async function toggleStatus(row: Row) {
    ...
    try {
      ...
      setRows((prev) =>
        prev.map((r) => (r.id === row.id ? { ...r, effective_status: newStatus, status: newStatus } : r)),
      )
    } catch (e) {
    ...
    try {
      const res = await fetch('/api/ads/meta/campaigns', {
        ...
        body: JSON.stringify({
          ...
          campaign_id: id,
          ad_account_id: adAccountId,
          daily_budget_cents: Math.round(amount * 100),
        }),
      })
  ...
  function onRowClick(row: Row) {
    if (view.level === 'campaigns') {
      setView({ level: 'adsets', campaign: { id: row.id, name: row.name } })
      ...
      <DialogContent className="max-w-6xl gap-0 p-0">
          ...
          <DialogTitle className="flex items-center gap-1.5 text-[14px]">
            ...
            {view.level !== 'campaigns' && (
              <>
                ...
                <button
                  onClick={() => setView({ level: 'adsets', campaign: view.campaign })}
                  className={cn(
            ...
            {view.level === 'ads' && (
              <>
                <ChevronRight className="h-3.5 w-3.5 text-text-tertiary" />
                <span className="max-w-[200px] truncate text-text-primary">{view.adset.name}</span>
              </>
            )}
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="max-h-[70vh] overflow-auto">
          {loading ? (
            ...
            <div className="m-5 flex items-center gap-2 rounded-lg border border-red-500/20 bg-red-500/10 px-4 py-3 text-[13px] text-red-400">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-1.5 truncate font-medium text-text-primary">
                            <span className="max-w-[240px] truncate">{row.name}</span>
                            {drillable && (
                              <ChevronRight className="h-3 w-3 shrink-0 text-text-tertiary" />
                            )}
                          </div>
                          {row.objective && (
                            <div className="text-[11px] text-text-tertiary">{row.objective}</div>
                          )}
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-2.5 text-right">
                      <span
                        className={cn(
                          ...
                          STATUS_COLORS[row.effective_status ?? ''] ?? STATUS_COLORS.ARCHIVED,
                        )}
                      >
                        ...
                        <td className="px-4 py-2.5" onClick={(e) => e.stopPropagation()}>
                          <div className="flex items-center justify-center gap-1">
                            <Button
                              ...
                              disabled={
                                mutating === row.id ||
                                !['ACTIVE', 'PAUSED'].includes(row.effective_status ?? '')
                              }
                            ...
                            <Button
                              ...
                              size="icon-sm"
                              onClick={() => {
                                setEditingBudget(row.id)
                                setNewBudget(
                                  row.daily_budget ? String(parseFloat(row.daily_budget) / 100) : '',
                                )
                              }}
