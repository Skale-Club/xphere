...
import type {
  AnalyticsMetrics, GeoRow, DeviceRow, RecentSession,
} from '@/lib/analytics/types'
import { trendPct } from '@/lib/analytics/queries'
...
export function DashboardView({ setup }: Props) {
  ...
  // Deep-link support: /analytics?settings=1 opens the settings modal directly
  // (also where the legacy /settings/traffic route redirects to).
  ...
  const m: AnalyticsMetrics | null = data?.metrics ?? null
    ...
    <div className="mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="ghost" size="sm" onClick={() => setSettingsOpen(true)} className="text-xs text-text-tertiary">
            Settings
          </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <TableCard title="Sources" rows={data?.sources ?? []} columns={[
