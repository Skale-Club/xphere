...
import { resetAnalyticsSetup, saveAnalyticsSetup } from '../actions'
...
> = {
  verified: {
    ...
    description: 'Data is flowing into your analytics dashboard.',
  },
  no_events_yet: {
...
export function SettingsDialog({
  ...
  const scriptSrc = `https://xphere.app/api/analytics/script?t=${scriptToken}`
  const gtmTag = `<script async src="${scriptSrc}"></script>`
  const headSnippet = `<!-- Xphere Analytics | place in <head> -->
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Manage your website tracking install, verify it again, or reset the setup.
          </DialogDescription>
        </DialogHeader>
