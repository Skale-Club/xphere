...
export default function NewSignaturePage() {
  ...
  function handleCreate(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const result = await createSignature(name)
      if (!result.ok) {
        toast.error(result.error)
        return
      }
      router.push(`/settings/signatures/${result.data.id}`)
    })
  }
  ...
  return (
    <div className="mx-auto w-full max-w-md px-4 py-16 space-y-6">
      <div className="text-center space-y-2">
        ...
        <h1 className="text-xl font-semibold">New Signature</h1>
        <p className="text-sm text-muted-foreground">
          Give your signature a name (e.g. Sales, Support). You&apos;ll build the HTML next.
        </p>
      </div>
      ...
      <form onSubmit={handleCreate} className="space-y-4">
        ...
        <Button type="submit" className="w-full gap-2" disabled={!name.trim() || isPending}>
          {isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Signature className="h-4 w-4" />}
