...
export async function getAnalyticsOverview(): Promise<AnalyticsPlatformMetrics> {
  ...
  const [setupsRes, pageviewsRes, sessionsRes, visitorsRes, recentSessionsRes] = await Promise.all([
    s.from('analytics_setups').select('id, verification_state'),
    s.from('analytics_pageviews').select('id', { count: 'exact', head: true }).gte('created_at', since30d),
    s.from('analytics_sessions').select('id', { count: 'exact', head: true }).gte('started_at', since30d),
    s.from('analytics_visitors').select('id', { count: 'exact', head: true }).gte('created_at', since30d),
    // Recent sessions with org info — top-orgs are aggregated from this window below
    s.from('analytics_sessions')
      .select(`
        id,
        organization_id,
        country_code,
  ...
  const top_orgs: AnalyticsOrgRow[] = Array.from(orgMap.entries())
